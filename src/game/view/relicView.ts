...
import { screenOffset } from '../layout';
...
export class RelicView {
  ...
  sync(world: World, camX: number, camY: number, viewW: number, viewH: number, dtMs: number): void {
    ...
    // both are scroll-free, so they take the offset that puts a screen position on the screen
    const off = screenOffset(this.scene);
    this.arrow.setVisible(true).setPosition(sx + off.x, sy + off.y).setRotation(angle).setScale(bob * (26 / this.arrow.width));
    this.icon.setVisible(true).setFrame(best.frame).setPosition(sx - Math.cos(angle) * 26 + off.x, sy - Math.sin(angle) * 26 + off.y).setDisplaySize(22, 22);
  }
